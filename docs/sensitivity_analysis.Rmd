...
I compiled 24 alternate data sets from the same spider phylogenies. All data sets and the files needed to produce them are included as data objects on this `R` package. For the time being, this package is private and access to the raw data is only possible via invite. 
...
Broadly speaking, the data consists of mainly singleton endemic species which colonised the island early. In many cases the colonisation time predates the island age. On scenarios favouring cladogenesis, 50 independent clades are present, while the number increases to 52 on scenarios favouring colonisation. There are 52 species in total. 7 of all species are known to be present as nonendemics on the island, but have not been sampled in the phylogeny, hence their colonisation time is unknown.
...
data("a_m_no", package = "hawaiispiders")
DAISIE::DAISIE_plot_island(a_m_no)
```
**Figures 1 to 4. Input island data plots with all colonising lineages. Fig 1. y_c_no scenario; Fig 2. o_c_no scenario; Fig 3. y_m_no scenario; Fig 4. o_m_no scenario**
...
**Table 2. Parameter estimates obtained for the best model of each scenario.**
```{r crdi results, echo=FALSE}
# best model results
best_model <- rep("cr_dd", 10)
row_names <- c()
...
## Discussion
The constant rate diversity independent model is chosen as the best model for nearly all scenarios, with the exception of the o_c_no scenario. A more thorough analysis is needed to confirm the validity of model fitting in this instance. 

Overall, there appears to be no signature of diversity dependence. Higher values of extinction are estimated in models favouring uncertain colonisation times and a higher island age, suggesting these assumptions can lead to different conclusions. Thus, care should be taken when fist constructing data sets so as to avoid at times erroneous interpretations. A thorough assessment of this impac ton these data may serve as illustrative example of the importance of model assumptions and data pre-processing.
