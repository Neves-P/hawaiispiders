...
```
Table 1. BIC for each model and dataset, when comparing between different island ages. Cells in red correspond to the lowest BIC value of a given dataset, and hence the preferred model. 
...
rate_sens <- list()
for (i in seq_along(rate_comparisons)) {
  cr_model_fits[[i]] <- DAISIEutils::sensitivity(
    rate_comparisons[[i]], full_output = TRUE
  )
}
cr_model_fits <- data.frame()
row_names <- c()
for (i in seq_along(rate_sens)) {
  for (j in seq_along(cr_model_fits[[i]][[4]])) {
    cr_model_fits <- rbind(
      cr_model_fits, rate_sens[[i]][[4]][[j]]$cr_di
    )
    row_names[i] <- names(rate_sens[[i]][[4]])
  }
}
...
Table 2. BIC for each model and dataset, when comparing between cladogenesis or colonisation preference. Cells in red correspond to the lowest BIC value of a given dataset, and hence the preferred model.
