...
rate_sens <- DAISIEutils::sensitivity(rate_comparisons, full_output = TRUE)

# rate_model_rankings <- data.frame(lapply(rate_sens$model_selection_rank, "[", 1))
rate_sens$model_selection_rank
...
for (i in seq_along(rate_sens)) {
  for (j in seq_along(rate_sens[[i]][[4]])) {
    cr_model_fits <- rbind(
      cr_model_fits, rate_sens[[i]][[4]][[j]]$cr_di
    )
  }
  
  row_names <- c(row_names, names(rate_sens[[i]][[4]]))
}
rownames(cr_model_fits) <- row_names
