...
DAISIE_plot_island <- function (island, island_age = NA) {
  ...
  data_to_plot <- prepare_island_plot_df(island)

  colours <- c("none" = "white", "empty" = "white", "filled" = "black")
  island_plot <- ggplot2::ggplot(data_to_plot) +
    ggplot2::geom_line(ggplot2::aes(x = Clade, y = Branching_times, color = stac)) +
    ggplot2::geom_point(
      ggplot2::aes(
        x = Clade,
        y = Branching_times,
